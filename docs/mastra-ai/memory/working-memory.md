...
While [conversation history](/docs/memory/conversation-history) and [semantic recall](/docs/memory/semantic-recall) help agents remember conversations, working memory allows them to maintain persistent information about users across interactions.
...
const memory = new Memory({
  ...
  options: {
    workingMemory: {
      enabled: true,
      scope: "resource", // Memory persists across all threads for the same resourceId
      template: `# User Profile
- **Name**:
- **Interests**:
- **Current Goal**:
...
- Long-term user preferences and profile information
- Cross-conversation context that should persist
- Building up knowledge about a user over multiple sessions
