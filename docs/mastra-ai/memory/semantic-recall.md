...
Semantic recall is RAG-based search that helps agents maintain context across longer interactions when messages are no longer within [recent conversation history](/docs/memory/conversation-history).
...
For more info on viewing message traces, see [Viewing Retrieved Messages](/docs/memory/overview#viewing-retrieved-messages).